...
/**
 * A scanner meant to detect charset meta tags in a byte stream See:
...
public class PreScanner {
    ...
    public Charset detectBOM() {
        try {
            if (expect(UTF8_BOM)) {
                this.bomSize = UTF8_BOM.length;
                return StandardCharsets.UTF_8;
            } else if (expect(UTF16_BE_BOM)) {
                this.bomSize = UTF16_BE_BOM.length;
                return StandardCharsets.UTF_16BE;
            } else if (expect(UTF16_LE_BOM)) {
                this.bomSize = UTF16_LE_BOM.length;
     ...
     *
     * @return the attribute as a Map.Entry, where the key is the attribute's name and the value is
     *         the attribute's value. If there is no attribute, return null
     */
    ...
    private String getAttributeName() throws IOException {
        ...
        while (!(peek() == EQUAL && name.length() > 0) && !(peek() == TAG_END || peek() == SLASH) && !skipAll(WHITESPACE)) {
    ...
    private String getAttributeValue() throws IOException {
        ...
        if (contains(QUOTE, quote)) {
            ...
            this.stream.reset(); // unread the space or tag end
        }
